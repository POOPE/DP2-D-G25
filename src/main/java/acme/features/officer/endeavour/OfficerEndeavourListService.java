/*
 * AnonymousEndeavourListService.java
 *
...
@Service
public class OfficerEndeavourListService implements AbstractListService<Officer, Endeavour> {

	// Internal state ---------------------------------------------------------

	@Autowired
	protected OfficerEndeavourRepository repository;

	// AbstractListService<Administrator, Endeavour> interface --------------
	...
	public Collection<Endeavour> findMany(final Request<Endeavour> request) {
		...
		return this.repository.findAll(Endeavour.withOfficer(request.getPrincipal().getAccountId()));
	}
